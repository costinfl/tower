 ...
 *
 * <p>The renderer is a pure function of the document: same document, same bytes.
 * That is what makes generated documentation disposable (IA-03) and
 * reproducible, and it is why no generation timestamp appears anywhere below.
 * Whoever regenerates this file tomorrow gets a clean diff, not a spurious one.
 *
...
public class MarkdownReleaseDocumentRenderer {
    ...
    public String render(ReleaseDocument document) {
        ...
        renderPromotionPath(out, document.promotionPath());
        renderContents(out, document.contents());
        renderHandover(out, document.handover());
        renderIterations(out, document.iterations());
        renderSightings(out, document.sightings());

        out.append("---\n\n")
                .append("_Generated by Tower from the Canonical Model. This document is a view of that model,\n")
                .append("not a source of truth (BR-07), and can be regenerated at any time._\n");
